...
class Helicase:
    ...
    def unwind(self):
        start = self.lead_node # Do i need this? why did i put this here?
        while self.lead_node:
            #print(self.lead_node)
            #we need to save lag_node where we will set the primer too, before we break the bond between leading and lagging nodes helicase processes
            lag_node = self.lead_node.bond.next
            if self.lead_node.bond:
                self.lead_node.bond = None
            self.count+=1
            if self.count % 3 == 0: #How will you set primer to Lag_Node if its None?
                if lag_node:
                    q = Primase(lag_node)
                    #print("*"*30)
                    q.set_primer()
...
class Primase: #This isn't particularly accurate since there is a protein that replaces primer
    ...
    def set_primer(self):
        primer = Node(self.complement_nuc,next=None,bond=self.lead_or_lag_node) #***its actually lead_or_lag_node that needs to set the bond
        #self.lead_or_lag_node.bond = primer
...
class Polymerase:
    ...
    def elongate(self):
        while self.origin:
        #****you will need to add a case were another primer is reached to stop so ligace can take action
            if self.origin.bond:
            #all the nodes in the origin are unbounded so when you encounter a bounded node you know you reached another primer
                l = Ligace(self.origin.bond,self.primer)
                l.ligate()
        ...
        while self.old_primer:
            print(self.old_primer,end="")
            self.old_primer = self.old_primer.next
        print()
...
class Ligace:
    ...
    def __init__(self,old_primer,primer):
        ...
        self.primer = primer
    def ligate(self):
        #print(">") #to test if ligate is being called and polymerase doesn't just override
...
origin_of_replication = a
h = Helicase(origin_of_replication) 
h.unwind()

#*******Note: i didn't check if when previous primer is encountered if polymerase overides pre existing strand as it follows origin
# i also didn't add logic to separate leading and lagging, just did lagging as its more complicated
